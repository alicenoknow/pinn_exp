...
def plot_initial_condition(save_path: str,
    ...
    fig2 = plot_3D_matplotlib(z, x, y, n_points_plot, length, domain,
                   f"{title} - exact", limit=limit, limit_wave=limit_wave)

    z = pinn(x, y, t)

    fig3 = plot_color(z, x, y, n_points_plot, f"{title} - PINN", limit=limit_wave)
    fig4 = plot_3D_matplotlib(z, x, y, n_points_plot, length, domain,
...
def plot_3D_matplotlib(z: torch.Tensor,
            x: torch.Tensor,
            y: torch.Tensor,
            n_points_plot: int,
            length: int,
...
def plot_3D(z: torch.Tensor,
    ...
    fig.update_layout(
        ...
        scene=dict(
            xaxis=dict(title="x"),
            yaxis=dict(title="y"),
            zaxis=dict(range=[limit[0], limit[1]]),
            camera=dict(eye=eye)
        ))

    if isinstance(domain, MeshDomain):
        fig.add_trace(go.Mesh3d(x=domain.x_raw,
                                y=domain.y_raw,
                                z=domain.z_raw,
                                opacity=1,
...
def plot_3D_top_view(z: torch.Tensor,
   ...
   return plot_3D(z, x, y, n_points_plot, domain, title, limit, limit_wave, eye=dict(x=0, y=0, z=2))
...
def plot_3D_side_view(z: torch.Tensor,
   ...
   return plot_3D(z, x, y, n_points_plot, domain, title, limit, limit_wave, eye=dict(x=1, y=2.5, z=0.5))
...
def plot_frame(save_path: str,
    ...
    fig2 = plot_3D_top_view(z, x, y, n_points_plot, domain,
    ...
    fig3 = plot_3D_side_view(z, x, y, n_points_plot, domain,
                             title, limit=limit, limit_wave=limit_wave)
